...
public class ARP {
    ...
    //target PC의 IP주소를 통해서 ARP Request 요청(MAC 주소 받아오기)
    public void makeARPRequest(byte[] sourceMAC, byte[] senderIP, byte[] targetIP)
    {
        Arrays.fill(destinationMAC, (byte)0xff); //0xff : broadcast 방식
        System.arraycopy(sourceMAC,0,this.sourceMAC, 0 ,6);
        opcode[0] = 0x00;
        opcode[1] = 0x01;  //request
        System.arraycopy(sourceMAC,0,this.senderMAC, 0 ,6);  // ARP request는 sourcemac 과 sendermac이 같아야함
